...
import { deleteProductInBasket, changSubmittingFormStatus } from "../store/slices/basket";
import { useState, useEffect } from "react";
import { makingOrder } from "../components/utils/utils";
...
export default function BasketPage() {
    ...
    const {productsInBasket, submittingFormStatus} = useSelector(state => state.basket);
  ...
  useEffect(() => {
    let newTotalCost = productsInBasket.reduce((sum, item) => sum + (item.product.price * item.quantity), 0);
  ...
  useEffect(() => {
    ...
    if(submittingFormStatus) return  <div className="submitting-form-ok">Заказ оформлен</div>
    return (
      ...
      <div className="basket">
        ...
        {productsInBasket? productsInBasket.map((el, i) =>(<div className="basket-items-title" key={el.product.id + el.size}>
            ...
            <div className="basket-item basket-item-price meaning">{el.product.price} руб.</div>
            <div className="basket-item basket-item-total meaning">{el.product.price*el.quantity} руб.</div>
            ...
            <div className="basket-total basket-total-text">Общая стоимость</div>
            <div className="basket-total basket-total-cost">{totalCost} руб.</div>
            </div>
      </div>
      ...
      <div className="making-order">
        <form className="making-order-form">
          ...
          <input  className="making-order-input" value={phone} onChange={handleChangePhone} type={"tel"} placeholder={"Ваш телефон"} id="making-order-form-tel"></input>
          <label htmlFor="making-order-form-adress">Адрес доставки</label>
          <input  className="making-order-input" value={address} onChange={handleChangeAddress} placeholder={"Адрес доставки"} id="making-order-form-adress"></input>
          <div className="checkbox-agree-container">
          <input className="checkbox-agree-marker" type="checkbox"  onChange={handleChangeCheckboxAgree}></input>
          <span className="checkbox-agree-text">Согласен с правилами доставки</span>
          </div>
          <div className="making-order-button" onClick={handleClickMakingOrder}>Оформить</div>
        </form>
      </div>
      </div>
    );
